...
def runCommand(command):
    ...
    try:
        ...
        print('Exception raised during execution of: \t' + command.strip())
...
def generate_FS_IDP_files(SUBJECTS_DIR, subject_ID, subject, dataDir):
    ...
    if os.path.isfile(statsDir + 'aseg.stats'):
        runCommand('$FREESURFER_HOME/bin/asegstats2table  '+\
        ...
        for value in ["volume", "area", "thickness"]:
            for atlas in ["BA_exvivo", "aparc.DKTatlas", "aparc.a2009s", "aparc"]:
                ...
                if os.path.isfile(statsDir + hemi + "." + atlas + '.stats'):
                    runCommand('$FREESURFER_HOME/bin/aparcstats2table '+\
                        ' -m ' + value + ' --hemi=' + hemi +\
    ...
    with open(os.environ["BRC_GLOBAL_DIR"]+'/config/FS_initial_files.txt') as f:
    ...
    for file_generated in files_generated:
        if os.path.isfile(dataDir + file_generated[0] + '.txt'):
            ...
            data = read_file(os.environ["BRC_GLOBAL_DIR"]+'/config/FS_data/' + file_generated[0] + '.txt')
...
def save_data(data_dict, SUBJECTS_DIR):
    ...
    with open(os.environ["BRC_GLOBAL_DIR"]+'/config/FS_headers.txt') as f:
    ...
    # Full string including ID (for saving to file)
    ...
    # Return string excluding the ID (assumed to be first item in headers)
    id_removed_str = " ".join(full_values[1:])
    return id_removed_str

def brc_FS_get_IDPs(direc, subject, IDP_folder_name):
    ...
    SUBJECTS_DIR = direc + '/' + subject + '/' + AnalysisFolderName + '/' + AnatMRIFolderName + '/' + T1FolderName + '/' + processedFolderName + '/'
    subjectDir=SUBJECTS_DIR + subject_ID
    dataDir = direc + '/' + subject + '/' + AnalysisFolderName + '/' + IDP_folder_name + '/'

    check_and_create_dir(dataDir)
    
    # print("1- generate_FS_IDP_files")
    data_dict = generate_FS_IDP_files(SUBJECTS_DIR, subject_ID, subject, dataDir)
    # print("2- fix_aseg_data")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 4:
        print("Usage: python brc_FS_get_IDPs.py <direc> <subject> <outdir>")
        sys.exit(1)
