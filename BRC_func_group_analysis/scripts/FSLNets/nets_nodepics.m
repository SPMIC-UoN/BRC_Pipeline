...
figure('visible' , 'off' , 'Position',[10 10 3*size(pic,2) 3*size(pic,1)]); subplot('Position',[0 0 1 0.9]); imagesc(pic); axis off; axis equal; title('good components');
...
if ts.NnodesOrig~=ts.Nnodes
  ...
  pic=imread(sprintf('%s.png',grot));
  figure('visible' , 'off' , 'Position',[10 100+3*size(pic,1) 3*size(pic,2) 3*size(pic,1)]); subplot('Position',[0 0 1 0.9]); imagesc(pic);  axis off; axis equal; title('bad components');
end
