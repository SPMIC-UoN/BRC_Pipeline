...
[grotA,grotB,grotC]=fileparts(sumpics); 

if size(grotA,1)==0 
    grotA='.'; 
end; 

sumpics=sprintf('%s/%s.sum',grotA,grotB);
...
figure('visible', 'off' , 'position' , [10 10 1000 500]);  clear y;  N=size(netmatL,1);  gap=.5/(N+1);
...
subplot('position',[gap 0 1-2*gap H1-0.01]); 
i=dpRSN;
grot=tril(netmatL(i,i)) + triu(netmatH(i,i)); 
grot=max(min(grot,0.95),-0.95);  
grot(eye(length(grot))>0)=Inf;
colormap('jet');
grotc=colormap;  
grotc(end,:)=[.8 .8 .8];  
colormap(grotc);  
...
if exist(grott) == 2
    grot=imread(grott);  
    subplot('position',[gap H1 1-2*gap H2-H1-0.035]); 
    imagesc(grot); 
    axis off; 
    system(sprintf('/bin/rm %s*',grott)); 
    daspect('auto');
end;
